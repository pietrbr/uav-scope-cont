...
std::string get_IP_from_IMSI(long long unsigned int ue_imsi, std::string fileName){
std::ifstream file(fileName.c_str());
//std::cout<< system("echo $PWD"); //find path of executable
std::string line;
  while (std::getline(file, line)){
      std::istringstream iss(line);
      std::string Name, Auth, imsi, Key, OP_Type, OP ,AMF ,SQN, QCI, ip_addr;
      if (std::getline(iss, Name, ',') &&
          std::getline(iss, Auth, ',') &&
          std::getline(iss, imsi, ',')&&
          std::getline(iss, Key, ',')&&
          std::getline(iss, OP_Type, ',')&&
          std::getline(iss, OP, ',')&&
          std::getline(iss, AMF, ',')&&
          std::getline(iss, SQN, ',')&&
          std::getline(iss, QCI, ',')&&
          std::getline(iss, ip_addr)){
          std::string ue_imsi_string=std::to_string(ue_imsi);
          ue_imsi_string=ue_imsi_string.insert (0, 2, '0'); // add 00 at the beginning of the string to be consistent with the file
          //std::cout<<"imsi: "<< imsi<<std::endl;
          //std::cout<<"ue_imsi_string"<<ue_imsi_string<<std::endl;
          if (imsi == ue_imsi_string){
              std::cout << "The ip is: " << ip_addr << std::endl;
              return ip_addr;
          }
      }
  }
  ...
  for (uint32_t ue_count = 0; ue_count < ue_db.size(); ++iter, ++ue_count) {
    ...
    if (user->imsi == 0) {
        ...
        if (ue_imsi > 0) {
            ...
            std::string ue_ip=get_IP_from_IMSI(ue_imsi, "user_db.csv");//--------------------------------------------------------
        }   
    }
